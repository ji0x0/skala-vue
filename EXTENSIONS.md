...
README의 과제 수행 내용과 추가 기능을 보완하는 구현 기록이다. 화면 구성, Store와 API 구조, 지표 계산, 배포 과정처럼 README에서 줄여 쓴 세부 내용을 정리한다.
...
- `App.vue`에 `RouterView`를 배치하고, Element Plus `el-menu` 항목에 `RouterLink`를 연결했다.
...
실습 1-3단계는 당시 결과물을 보완한 컴포넌트를 렌더링한다. 화면 결과물이 따로 없던 4-8단계에는 라우터·Store·API·UI·빌드 설정을 확인할 수 있는 데모를 추가했다.

| 단계 | 실행 화면 | 내용 |
| --- | --- | --- |
| 1 | `WeatherMockup` | 목업 요구사항과 개인 확장 기능 |
| 2 | `WeatherComposition` | 반응형 요구사항과 추가 상태·Computed·Watcher |
| 3 | `WeatherParent` | 컴포넌트 분리 결과와 보완한 scoped 스타일 |
...
다크 모드는 Element Plus의 `theme-chalk/dark/css-vars.css`를 불러오고 `html`에 `dark` 클래스를 토글하는 방식이다. 직접 작성한 스타일의 주요 색상을 Element Plus CSS 변수(`--el-bg-color`, `--el-text-color-primary`, `--el-border-color` 등)로 바꿨다. 배지처럼 고정 강조색을 쓰는 요소는 흰색 글자를 유지했다.
...
대기질은 Open-Meteo의 `european_aqi` 대신 PM10·PM2.5 농도를 국내 CAI 구간에 대입해 표시한다. 전체 CAI 오염물질과 복합오염 보정은 포함하지 않은 간이 지수다.
...
Vue 프론트엔드는 `dist/` 정적 파일로 배포하고, 키가 필요한 API만 같은 도메인의 서버리스 함수로 제공한다. 전체 구성은 정적 프론트엔드와 서버리스 API를 함께 쓰는 형태다.
...
읽은 값은 `process.env`에만 넣는다. 로컬에서도 배포용 `api/*.js`를 불러오므로 `npm run dev`로 API 흐름을 확인할 수 있다.
...
- 서버리스 함수 3개(`api/openweather`, `api/kpx`, `api/opinet`)가 배포에 포함된다.
...
| --- | --- |
| 로그인 없이 접속 | 정상 |
| 키가 필요한 API | 브라우저 요청 주소에 키가 포함되지 않음 |
| 대시보드 4개 카드 | 오류 없이 실데이터 표시 |
| 주소 직접 입력 접속 | `/weather/city_01`, `/solar/ulsan`, `/labs/8` 확인 |
...
아래 항목은 완료 여부와 별개로 최종 제출본에서 한 번 더 확인한다.

- Git 이력의 비밀값 포함 여부
- ESLint와 프로덕션 빌드
- 내비게이션, 검색, 설정, API 오류 처리, 404
- 로그인하지 않은 상태의 공개 저장소 접근
