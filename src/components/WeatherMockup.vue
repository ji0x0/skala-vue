...
<template>
    <div class="dashboard-wrapper">
        ...
        <section class="search-box">
            ...
            <!-- 입력한 도시 정보가 없을 때 -->
            <p v-else-if="!isCityFound()"><strong>{{ searchQuery }}</strong>의 날씨 정보를 알 수 없습니다.</p>
            <!-- 입력한 도시 정보가 있을 때 -->
            <p v-else><strong>{{ searchQuery }}</strong>의 날씨 정보는 확인 가능합니다.</p>
        </section>

        <!-- 날씨 카드 목록-->
         <section class="weather-card-list">
            <h3> 지역별 날씨 현황 </h3>
            <div v-for="item in weatherlist" :key="item.id" class="weather-card" @click="selectedCityInfo = `${item.city}이 선택되었습니다.`">
        ...
        <section class="weather-compare">
            ...
            <!-- 두 도시를 모두 선택하지 않은 경우 -->
            <p v-if="leftCity === '' || rightCity === ''"> 
            비교할 두 도시를 선택하세요.
            </p>
            <!-- 같은 도시를 선택한 경우 -->
            <p v-else-if="leftCity === rightCity">
            서로 다른 도시를 선택하세요.
            </p>
            <!-- 서로 다른 도시를 선택한 경우 -->
            <div v-else class="compare-result">
                <div class="compare-card">
                    ...
                    <p>
                    날씨:{{ getSelectedCity(leftCity).condition }}
                    </p>
                    <p>
                    기온:{{ getSelectedCity(leftCity).temp }}°C
                    </p>
                    <p>
                    습도:{{ getSelectedCity(leftCity).humidity }}%
                    </p>
                    <p>
                    풍속:{{ getSelectedCity(leftCity).wind }}km/h
                    </p>
                    </div>
            <div class="compare-card">
                <h4>{{ getSelectedCity(rightCity).city }}</h4>
                <p>
                날씨:{{ getSelectedCity(rightCity).condition }}
                </p>
                <p>
                기온:{{ getSelectedCity(rightCity).temp }}°C
                </p>
                <p>
                습도:{{ getSelectedCity(rightCity).humidity }}%
                </p>
                <p>
                풍속:{{ getSelectedCity(rightCity).wind }}km/h
                </p>
            </div>
            </div>
        </section>
